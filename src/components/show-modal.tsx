        ...
        <div className="relative aspect-video">
          ...
          <div className="absolute bottom-6 z-20 flex w-full items-center justify-between gap-2 px-10">
            <div className="flex items-center gap-2.5">
              ...
              <Button
                aria-label="Watch show now"
                className="group h-auto rounded py-1.5"
                onClick={() => {
                  if (!modalStore.show?.slug) return
                  setOpen(false)
                  modalStore.setShow(null)
                  router.push(`/movie/${modalStore.show.slug}`)
                }}
                disabled={!modalStore.show?.slug}
              >
                <Icons.play
                  className="mr-1.5 h-6 w-6 fill-current"
                  aria-hidden="true"
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    myShowsQuery?.data?.find(
                      (item) => String(item.id) === String(modalStore.show?.id)
                    )
